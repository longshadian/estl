...
void Test3()
{
    std::string url = "http://127.0.0.1:6071/test/download/download.json";
    HttpDownload http{};
    //std::string s = R"(D:\cmake_builds\libcurl_builds\download\xxx)";
    std::string s = R"(D:\vspro\httpdownload\Download\x64\Debug\download)";
    assert(http.Init(s));
    http.Launch(url);
}
...
int main()
{
    ...
    //std::string s = "abc";
    //std::string ss;
    //std::transform(s.begin(), s.end(), std::back_inserter(s), std::bind(&std::toupper, std::placeholders::_1));
