...
int SrsRtmp::SendH264(void* frameData, size_t frameLen, uint64_t pts)
{
    int ret = 0;
    if (!hdl_)
        ret = -1;
...
#else
    ret = srs_h264_write_raw_frames(hdl_, (char*)frameData, (int)frameLen, (u_int32_t)pts, (u_int32_t)pts);
#endif
    if (ret != 0) {
