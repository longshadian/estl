...
def select_battle_log(conn, cnt, tm):
    try:
        cursor = conn.cursor()
        sql = "select count(cnt) from (select count(user_id) as cnt from answer_log_20181228 where date(createtime) = '{tm}' group by user_id) as x where x.cnt = {idx}".format(tm=tm,idx=cnt)
        cursor.execute(sql)
...
def select_battle_log_total(conn, tm):
    try:
        cursor = conn.cursor()
        sql = "SELECT count(distinct(user_id)) FROM answer_log_20181228 where date(createtime) = '{tm}'".format(tm=tm)
        cursor.execute(sql)
        rows = cursor.fetchall()
        for row in rows:
            print("总人数", row[0])
...
def select_battle_log_total_count(conn, tm):
    try:
        cursor = conn.cursor()
        sql = "SELECT count(id) FROM answer_log_20181228 where date(createtime) = '{tm}'".format(tm=tm)
        cursor.execute(sql)
        rows = cursor.fetchall()
        for row in rows:
            print("总次数", row[0])
