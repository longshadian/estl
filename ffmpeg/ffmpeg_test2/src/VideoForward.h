...
struct VideoFrame
{
    ...
    void Append(const char* p, size_t len, uint32_t pts)
    {
        buff.assign(p, p + len);
        pts_ = pts;
    }
...
protected:
    bool Init_rtsp();
    bool Init_rtmp();
    static void FrameProc0(unsigned char* buffer, unsigned int buffer_length, unsigned numTruncatedBytes, struct timeval presentationTime, unsigned durationInMicroseconds, void* private_data);

    virtual void FrameProc(unsigned char* buffer, unsigned int buffer_length, unsigned numTruncatedBytes, struct timeval presentationTime, unsigned durationInMicroseconds);
