...
bool VideoForward::Init_rtsp()
{
    ...
    params.frame_proc = &VideoForward::FrameProc0;
    params.frame_proc_private_data = this;
    if (!rtsp_receiver_->Init(params)) {
        std::cout << "init error\n";
        return false;
    }
    return true;
...
{
    VideoForward* pthis = reinterpret_cast<VideoForward*>(private_data);
    pthis->FrameProc(buffer, buffer_length, numTruncatedBytes, presentationTime, durationInMicroseconds);
}
