...
public:
    ...
    ByteBuffer(std::vector<uint8_t> buf) : m_rpos(0), m_wpos(0),
        m_storage(std::move(buf)) { }

    ByteBuffer(ByteBuffer&& buf) : m_rpos(buf.m_rpos), m_wpos(buf.m_wpos),
        m_storage(std::move(buf.m_storage)) { }

    ByteBuffer(ByteBuffer const& right) : m_rpos(right.m_rpos), m_wpos(right.m_wpos),
        m_storage(right.m_storage) { }

    ByteBuffer(const void* src, size_t len) : m_rpos(0), m_wpos(0),
        m_storage(static_cast<const uint8_t*>(src),
