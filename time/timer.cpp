...
int main(int argc, char** argv)
{
    ...
    Timer t{std::chrono::seconds(-2) + std::chrono::milliseconds(600)};
    SteadyTimePoint previous_tp = getStreadyTimePoint();
    SteadyTimePoint current_tp{};
    while (true) {
        current_tp = getStreadyTimePoint();
    ...
    TimerManager tm;
    tm.m_timer.push_back({Function{}, Timer{std::chrono::seconds(10)}});
    tm.m_timer.push_back({Function{}, Timer{std::chrono::seconds(2)}});
    tm.m_timer.push_back({Function{}, Timer{std::chrono::seconds(31)}});
    tm.m_timer.push_back({Function{}, Timer{std::chrono::seconds(2)}});
    tm.m_timer.push_back({Function{}, Timer{std::chrono::seconds(5)}});
    tm.m_timer.push_back({Function{}, Timer{std::chrono::seconds(6)}});
    tm.m_timer.push_back({Function{}, Timer{std::chrono::seconds(1)}});
