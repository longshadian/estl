...
public:
    ...
    GettingFrameProc proc_;
    void* proc_private_data_;
    bool shutdown_;
};
...
void continueAfterSETUP(RTSPClient* rtspClient, int resultCode, char* resultString) {
    do {
...

        {
            ...
            psink->proc_private_data_ = pc->proc_private_data_; 
        }
...
#endif
    if (proc_)
        proc_(fReceiveBuffer, frameSize + 4, numTruncatedBytes, presentationTime, durationInMicroseconds, proc_private_data_);
...
using namespace rtsp_poller_impl;
...
private:
    ...
    void TimeoutProc();
    void SetParams(const RtspPollerParams& argv);
...
private:
    ...
    GettingFrameProc frame_proc_;
    void* frame_proc_private_data_;
...
/* class RtspPoller                                                     */
/************************************************************************/
RtspPoller::RtspPoller()
    : impl_(new RtspPollerImpl())
{
}
...
bool RtspPoller::Init(const RtspPollerParams& argv)
{
    return impl_->Init(argv);
}

void RtspPoller::Loop()
{
    return impl_->Loop();
}
...
bool RtspPollerImpl::Init(const RtspPollerParams& argv)
{
    SetParams(argv);

    m_scheduler_ = BasicTaskScheduler::createNew();
    m_env_ = BasicUsageEnvironment::createNew(*m_scheduler_);
    m_our_client_ = ourRTSPClient::createNew(*m_env_, uri_.c_str(), 0, "RtspPoller");
    if (!m_our_client_) {
        StoreErrorInfo(m_env_->getErrno(), m_env_->getResultMsg());
        return false;
    }

    m_our_client_->proc_ = frame_proc_;
    m_our_client_->proc_private_data_ = frame_proc_private_data_;
    ...
    m_token_ = m_env_->taskScheduler().scheduleDelayedTask(1000000 * 1, &RtspPollerImpl::TimeoutProc0, this);
#endif
    return true;
}
...
void RtspPollerImpl::SetParams(const RtspPollerParams& argv)
{
    ...
    frame_proc_private_data_ = argv.frame_proc_private_data;
}
